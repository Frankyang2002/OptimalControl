function foot_monopod_opt()
    ...
    opti.subject_to(0.1 <= T_f1 <= 3.5); 
    opti.subject_to(0.1 <= T_s <= 2.5);  
    opti.subject_to(0.1 <= T_f2 <= 3.5); 
    ...
    alpha = 2e4;
    e = Xf2(2:8, N3+1) - Xf1(2:8,1);  % final state - initial state, Punish state errors
    ...
    opti.subject_to(Xf1(2,1) == y_max);%y, starts at 1m
    
    opti.subject_to(Xf1(3,1) == deg2rad(60));%q1 
    opti.subject_to(Xf1(4,1) == deg2rad(270));%q2 

    opti.subject_to(Xf1(5,1) == x_speed);%dx, Initial x vel
    ...
    % ymax is peak of jump
    
    opti.subject_to(Xf1(2, :) <= y_max);
    opti.subject_to(Xs(2, :) <= y_max);
    opti.subject_to(Xf2(2, :) <= y_max);

    opti.subject_to(Xf1(2, :) >= y_min);
    opti.subject_to(Xs(2, :) >= y_min);
    opti.subject_to(Xf2(2, :) >= y_min);

    % start == end
    opti.subject_to(Xf1(2, 1) - 0.1 <= Xf2(2, N3+1) <= Xf1(2, 1) + 0.1);
    
    % max distance
    opti.subject_to(Xf2(1, N3+1) <= 6);
    ...
    % x = [x; y; q1; q2; dx; dy; dq1; dq2]
    opti.subject_to(Xf1(5,:) == x_speed);
    opti.subject_to(0 <= Xs(5,:));
    opti.subject_to(x_speed - 0.5 <= Xf2(5, :) <= x_speed + 0.5);
